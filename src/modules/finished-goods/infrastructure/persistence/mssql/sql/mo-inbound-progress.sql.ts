...
inbound_epcs AS (
   SELECT EPC_Code, size_code AS size_numcode
   FROM DV_DATA_LAKE.dbo.dv_InvRFIDrecorddet
   WHERE mo_no = @0 
		AND EPC_Code NOT IN (SELECT EPC_Code FROM incomming_inbound_epcs)
		AND RIGHT(stationNO, 5) = 'WH101' 
		AND rfid_status = 'A' 
		AND isactive = 'Y' 
   UNION ALL
   SELECT EPC_Code, size_code AS size_numcode
...
SELECT 
	...
	a.size_qty AS size_qty,
	ISNULL(b.inbound_qty, 0) AS accumulated_qty
FROM mo_size_qty a
LEFT JOIN inbound_qty b ON a.size_numcode = TRY_CAST(b.size_numcode AS FLOAT)
`
