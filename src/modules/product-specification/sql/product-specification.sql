SELECT DISTINCT a.brand_name, JSON_QUERY(b.product_variants) AS product_variants 
FROM wuerp_vnrd.dbo.ta_brand a
OUTER APPLY (
   SELECT (
      SELECT DISTINCT shoestyle_codefactory AS shoes_style, JSON_QUERY(cl.spec) AS spec
      FROM wuerp_vnrd.dbo.ta_shoefactorymst b
      INNER JOIN wuerp_vnrd.dbo.ta_productmst c       
      ON b.shoestyle_systemcodefty = c.shoestyle_systemcodefty 
         AND c.isactive = 'Y'
         AND c.color_sn IS NOT NULL
      OUTER APPLY (
         SELECT (
         SELECT DISTINCT c.color_sn, JSON_QUERY(s.sizes) AS sizes
         ...
         OUTER APPLY ( 
            SELECT (
               ...
               WHERE p.color_sn = c.color_sn  
                  AND sz.size_numcode IS NOT NULL
                  AND sz.size_qty > 0
                  AND p.custbrand_id = a.custbrand_id
                  AND p.shoestyle_systemcodefty = b.shoestyle_systemcodefty
                  AND m.created >= CAST(DATEADD(YEAR, -2, GETDATE()) AS DATE)
               FOR JSON PATH
            ) AS sizes
         ) s (sizes)
         WHERE c.shoestyle_systemcodefty = b.shoestyle_systemcodefty AND s.sizes IS NOT NULL
         FOR JSON PATH
         ) AS spec
      ) cl (spec)
      WHERE b.custbrand_id = a.custbrand_id AND b.isactive = 'Y' AND cl.spec IS NOT NULL 
      AND b.created >= CAST(DATEADD(YEAR, -2, GETDATE()) AS DATE)
   FOR JSON PATH
   ) AS product_variants
) b (product_variants)
WHERE b.product_variants IS NOT NULL
AND a.isactive = 'Y'
AND a.brand_code IN ('TV','KB','UG')
