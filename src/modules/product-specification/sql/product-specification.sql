SELECT DISTINCT a.brand_name, CAST(b.product_variants AS NVARCHAR(MAX)) product_variants FROM wuerp_vnrd.dbo.ta_brand a
OUTER APPLY (
   SELECT (
      SELECT DISTINCT shoestyle_codefactory AS factory_shoes_style, CAST(cl.spec AS NVARCHAR(MAX)) spec
      ...
      OUTER APPLY (
         SELECT (SELECT DISTINCT c.color_sn, s.sizes
