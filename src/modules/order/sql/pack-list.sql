...
SELECT
   ...
   FROM ta_packlistdet b
      LEFT JOIN ta_ordermst o ON o.isactive= 'Y'
         AND o.or_no= b.or_no
      LEFT JOIN ta_manufacturdet md ON md.isactive= 'Y'
...
FROM
   ta_packlistmst AS a
   LEFT JOIN ta_packlistdet o ON o.isactive= 'Y'
      AND a.kg_no= o.kg_no
   LEFT JOIN ta_ordermst o1 ON o1.isactive= 'Y'
      AND o.or_no= o1.or_no
   LEFT JOIN ta_productmst h ON h.isactive = 'Y'
      AND o1.mat_code = h.mat_code
   LEFT JOIN ta_shoefactorymst ss ON ss.isactive= 'Y'
      AND h.shoestyle_systemcodefty = ss.shoestyle_systemcodefty
   LEFT JOIN ta_brand b ON b.isactive= 'Y'
      AND a.custbrand_id= b.custbrand_id
   LEFT JOIN ta_deliveryport dep ON dep.isactive= 'Y'
