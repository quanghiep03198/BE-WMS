SELECT 
   ordp.PO AS po,
   ordp.mo_no,
   ordp.brand AS brand_name,
   CONCAT(ordp.shoestyle_codecust, '/', ordp.shoestyle_codefactory) AS shoes_style,
   ordp.color_sn,
...
LEFT JOIN wuerp_vnrd.dbo.ta_ordermst ord
   ...
   AND a.isactive = 'Y'
   AND ord.isactive = 'Y'
INNER JOIN DV_DATA_LAKE.dbo.dv_OrderProcessR05 ordp 
   ON ordp.PO = IIF(ISNULL(ord.or_custpoone, '') = '', ord.or_custpo, ord.or_custpoone)
CROSS APPLY (
   VALUES
...
GROUP BY 
   ...
   ordp.shipid,
   ordp.ship_type,
   CONCAT(ordp.shoestyle_codecust, '/', ordp.shoestyle_codefactory),
   CASE 
      WHEN ISNUMERIC(b.size_numcode) = 1 THEN CAST(b.size_numcode AS NVARCHAR) 
      WHEN LEFT(b.size_numcode, 1) IN ('T', 'K') THEN SUBSTRING(b.size_numcode, 2, LEN(b.size_numcode))
   END