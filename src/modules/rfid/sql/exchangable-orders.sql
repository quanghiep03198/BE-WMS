...
WITH
    ...
    (
        ...
        WHERE inv.record_time >= CAST(GETDATE() AS DATE)
            AND inv.rfid_status IS NULL
            AND COALESCE(inv.mo_no_actual, inv.mo_no,  @UnknownValue) IN (@0, @1)
    )
...
WHERE d1.mo_no = @0
    ...
    WHERE d2.mo_no = @1
        AND d1.size_numcode = d2.size_numcode
)
ORDER BY d1.size_numcode ASC;
