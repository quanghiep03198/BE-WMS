DECLARE @FallbackValue AS NVARCHAR (10) = 'Unknown';

-- * Retrieves inbound report data around last 2 years
WITH     rfid_inbound_cte
AS       (SELECT EPC_Code,
                 ...
                 record_time,
                 stationNO,
                 FC_server_code AS factory_code,
                 ISNULL(dept_name, @FallbackValue) AS dept_name,
                 storage,
                 quantity
...
AS       (SELECT   mo_no,
          ...
          FROM     (SELECT DISTINCT storage,
                                    mo_no,
                                    dept_name,
                                    factory_code
                    FROM   inbound_detail_cte) AS a
                   LEFT OUTER JOIN
                   DV_DATA_LAKE.dbo.dv_warehouseccodedet AS b
...
SELECT   ric.factory_code,
         ...
         (SELECT   size_numcode,
                   COUNT(DISTINCT EPC_Code) AS qty
          FROM     rfid_inbound_cte AS d
          WHERE    d.mo_no = ric.mo_no
                   AND d.dept_name = ric.dept_name
                   AND CAST (d.record_time AS DATE) = @1
          GROUP BY mo_no, dept_name, size_numcode
          FOR      JSON PATH) AS size_data
FROM     rfid_inbound_cte AS ric
         LEFT OUTER JOIN
         DV_DATA_LAKE.dbo.dv_rfidmatchmst_cust AS rmc
         ON ric.EPC_Code = rmc.EPC_Code
         LEFT OUTER JOIN
         wuerp_vnrd.dbo.ta_manufacturmst AS manf
         ON manf.mo_no = ric.mo_no
         LEFT OUTER JOIN
         wuerp_vnrd.dbo.ta_productmst AS prod
         ON rmc.mat_code = prod.mat_code
         LEFT OUTER JOIN
         storage_list_cte AS slc
         ON slc.mo_no = ric.mo_no
            AND slc.factory_code = ric.factory_code
            AND slc.dept_name = ric.dept_name
         LEFT OUTER JOIN
         accumulated_cte AS ac
         ON ac.mo_no = ric.mo_no
WHERE    CAST (ric.record_time AS DATE) = @1
GROUP BY ric.factory_code, ric.mo_no, rmc.shoestyle_codefactory, prod.color_sn, manf.mo_totalqty, ac.accumulated_qty, slc.storage_name, ric.dept_name
ORDER BY ric.mo_no, ric.dept_name
