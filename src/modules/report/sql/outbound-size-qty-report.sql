WITH datalist AS (
	SELECT EPC_Code, mo_no_actual, mo_no, rfid_status, created, FC_server_code, stationNO FROM DV_DATA_LAKE.dbo.dv_InvRFIDrecorddet
	UNION ALL 
	SELECT EPC_Code, mo_no_actual, mo_no, rfid_status, created, FC_server_code, stationNO 
...
WHERE d.mo_no = @0 
   ...
   AND CAST(d.created AS DATE) = CAST(@2 AS DATE)
	AND d.rfid_status = 'B'
