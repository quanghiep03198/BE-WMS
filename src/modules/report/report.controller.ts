...
export class ReportController {
	...
	@AuthGuard()
	async getDailyPackingReport(@Query('date.eq', new DefaultValuePipe(format(new Date(), 'yyyy-MM-dd'))) date: string) {
		return await this.packingWeightReportService.getDailyPackingReport(date)
	}
	...
	async exportPackingWeightReport(
		...
		const buffer = await this.packingWeightReportService.exportDailyPackingToExcel(date)
		return res.send(buffer)
	}
}
